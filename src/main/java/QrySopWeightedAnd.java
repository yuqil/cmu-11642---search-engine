        ...
        if (! this.docIteratorHasMatchCache()) {
            ...
            for (int i = 0; i < this.args.size(); i ++) {
                ...
                if (qry.docIteratorHasMatch(r) && qry.docIteratorGetMatch() == this.docIteratorGetMatch()) {
                    ...
                    p_and *= Math.pow(s_qry.getScore(r), weight.get(i) / total_wight);
                } else {
                    QrySop s_qry = (QrySop) qry;
                    p_and *= Math.pow(s_qry.getDefaultScore(r, this.docIteratorGetMatch()), weight.get(i) / total_wight);
                }
            }
            return p_and;
        }
    ...
    public double getDefaultScore(RetrievalModel r1, int docid) throws IOException {
        ...
        for (int i = 0; i < this.args.size(); i ++) {
            ...
            if (qry.docIteratorHasMatch(r) && qry.docIteratorGetMatch() == docid) {
                ...
                QrySop s_qry = (QrySop) qry;
                p_and *=  Math.pow(s_qry.getDefaultScore(r, docid), weight.get(i) / total_wight);
            }
        }
        return p_and;
    }
